...
def do_colorization( image_gray, gray_output_dir='./img_dir', rgb_input_dir='./store_dir/final' ):
    ...
    for idx in range( r*c ):
        imageio.imwrite( f'{gray_output_dir}/{str(idx).zfill(8)}.png', np.asarray(_ig[idx], dtype='uint8' ) )
    ...
    command = f'python3 ./custom_colorize.py --batch_size=1 --config=configs/colorizer.py --logdir=./coltran/colorizer --img_dir=./img_dir --store_dir=./store_dir --mode=colorize && python3 ./custom_colorize.py --batch_size=1 --config=./configs/color_upsampler.py --logdir=./coltran/color_upsampler --img_dir=./img_dir --store_dir=./store_dir --gen_data_dir=./store_dir/stage1 --mode=colorize && python ./custom_colorize.py --batch_size=1 --config=./configs/spatial_upsampler.py --logdir=./coltran/spatial_upsampler --img_dir=./img_dir --store_dir=./store_dir --gen_data_dir=./store_dir/stage2 --mode=colorize'
    os.system( command )
    ...
    for idx in range( r*c ):
        _img = imageio.imread( f'{rgb_input_dir}/{str(idx).zfill(8)}.png' ) # RGBA image ....
        rgb_images[idx] = _img[:,:,:3]
...
def rescale( image ):
    image = image*1.0
    image -= np.amin(image)
    image /= np.amax(image)+1.0e-10
    return np.asarray( image*255.0, dtype='uint8' )


def colorize( input_gray_image_path, output_rgb_image_path, downscale=4 ):
    gray_image = imageio.imread( input_gray_image_path )
    print( f'opened gray image from {input_gray_image_path}, shape is {gray_image.shape}' )

    # pre-processing
    row, col = gray_image.shape
    gray_image = cv2.resize( gray_image, dsize=(row//downscale, col//downscale), interpolation=cv2.INTER_CUBIC )
    gray_image = rescale( gray_image )

    imageio.imwrite( output_rgb_image_path, do_colorization( gray_image ) )


if __name__ == '__main__':
    #colorize(  '/home/feng/Downloads/Session4-Job 010- Setup 006-face_1_high_32.png',  '/home/feng/Downloads/Session4-Job 010- Setup 006-face_1_high_32_rgb.png' )
    colorize( '/home/feng/Downloads/test_1_con.png', '/home/feng/Downloads/test_1_con_rgb.png' )
